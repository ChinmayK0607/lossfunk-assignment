...
"""
    ...
    # ––– replace the old similarity_reward_func with this ––––––––––––––––––
    ...
    def _avg_nll(model, tokenizer, prompt_txt: str, reasoning_txt: str,
        ...
        **truncating** so total length ≤ max_len.
        ...
        # 1) Tokenise with truncation
        toks = tokenizer.encode(
            seq,
            add_special_tokens=False,
            truncation=True,
            max_length=max_len - 1,      # leave room for BOS that model adds
            return_tensors="pt"
        ).to(model.device)

        # 2) Forward pass
        with torch.no_grad():
            logits = model(toks).logits[:, :-1]          # shift left
        
        logits = model(toks[:, :-1]).logits
        loss = F.cross_entropy(
            logits.reshape(-1, logits.size(-1)),
            labels.reshape(-1),
            ignore_index = tokenizer.unk_token_id,
            reduction    = "mean",
        )
        return loss        # average NLL


    def perplexity_reward_func(prompts, completions,
                            reference_reasoning,  # <-- list[str] from dataset
                            model=model, tokenizer=tokenizer, **_):

        rewards = []
        for p, comp, ref_r in zip(prompts, completions, reference_reasoning):
            prompt_txt = "".join([m["content"] for m in p])
            ...
            # positive if model finds its own reasoning ≥ as plausible as teacher’s
            delta   = nll_ref - nll_gen          # >0 is good
            reward  = torch.tanh(delta).item()   # smooth normalisation to (-1, 1)
